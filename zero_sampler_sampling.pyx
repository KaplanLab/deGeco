...
cdef class ZeroSampler:
    ...
    def _count_zeros(self):
        ...
        cdef int[::1] cols
        cdef long nn_cols, available_cols = non_nan_mask.size
        ...
        for i in range(self.nbins):
            ...
            if sample_area == SAMPLE_TRANS:
                ...
                while s < cis_offsets[chr_assoc[i]]:
                    s += 1
            cols = bin2_id[s:e]
            nn_cols = cols.size #self._non_nan_mask.base[cols].sum()
    ...
    def sample_zeros(self, n):
        ...
        nan_cols = set() #set(np.nonzero(self._non_nan_mask.base == 0)[0])
