...
if [ "$1" = --help ]; then
    ...
    echo
    echo Output files will be put in an output/ directory. Log files will be written to a logs/ directory.
...
for s in ${sampling_rates[@]}; do
    ...
    binned_mcool=output/${binned_basename}.mcool
    ...
    python -u downsample.py -m "$mcool_filename" -c "$chr" -r ${base_res} -o output/${baseres_basename}.cool -s $s --seed 1 --balance no > logs/${baseres_basename}.log 2>&1
    echo fraction=$s: Binning and balancing
    cooler zoomify -o $binned_mcool -p 5 -c 5000000 --balance --balance-args '--max-iters 10000' -r "$res_joined"  "output/${baseres_basename}.cool" > logs/${binned_basename}.log 2>&1
    cooler balance $binned_mcool::/resolutions/$base_res --max-iters 10000 --force >> logs/${binned_basename}.log 2>&1
...
for s in ${sampling_rates[@]}; do
            ...
            for r in ${resolutions_desc[@]}; do
                fit_basename=${chr}_${st}st_s${s}_${r}_run${i}
                    if [ -z "$previous_basename" ]; then
                        ...
                        stretch_args="--init output/${previous_basename}.npz --init-stretch-by $((previous_r/r))"
                    fi
                python gc_model_main.py -m $binned_mcool -ch ${chr} -kb ${r} -o output/${fit_basename}.npz -n $st --seed $i --iterations 1 --no-overwrite $stretch_args > logs/${fit_basename}.log 2>&1
    ...
    for r in ${resolutions[@]}; do
        ...
        python get_best.py output/${fit_basename}_run*.npz --target "output/${fit_basename}_best.npz"
    done
done
wait
