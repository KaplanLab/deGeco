...
def calc_optimal_stats_data_vs_resampled(st, resolutions, sample_rates):
    pearson_corrs = np.empty((len(resolutions), len(sample_rates)))
    spearman_corrs = np.empty((len(resolutions), len(sample_rates)))
    lr = np.empty((len(resolutions), len(sample_rates)))
    normalized_pearsonr = np.empty((len(resolutions), len(sample_rates)))
    for i, res in enumerate(resolutions):
        ...
        for j, pct in enumerate(sample_rates):
            ...
            print("done")
            simulated_mats = batch_load(f'/srv01/technion/hagaik/storage/paper/figure2/resampled_data_D_noallzeros//chr19_{st}st_{res}_{float(pct)}_run*.npy')
            ...
            stats = list(map(get_stats(input_mat), simulated_mats))
...
optimal_stats = calc_optimal_stats_data_vs_resampled(st, resolutions, sample_rates)
np.savez('/srv01/technion/hagaik/storage/paper/figure2/optimal_stats_arrow_D.npz', **optimal_stats)
