...
cdef class ZeroSampler:
    ...
    def __init__(self, long nbins, int[::1] bin1_id, int[::1] bin2_id):
        ...
        self.holes = np.empty(bin1_id.shape[0]+1, dtype=np.int64)
    ...
    def sample_zeros(self, n):
        ...
        for i in range(self._bin1_id.shape[0]):
            ...
            self.holes[i] = prev_holes + self._gap_size(prev_x, prev_y, cur_x, cur_y)
            prev_x = cur_x
            prev_y = cur_y
            prev_holes = self.holes[i]
        i = self._bin1_id.shape[0]
        self.holes[i] = prev_holes + self._gap_size(prev_x, prev_y, self.nbins-1, self.nbins)
